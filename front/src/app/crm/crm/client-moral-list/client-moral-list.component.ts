import { Component, inject, OnInit } from '@angular/core';
import { CommonModule } from '@angular/common';
import { Router } from '@angular/router';
import { ClientMoralService } from '../../../services/client-moral.service';
import { ClientMoral, ClientStatus } from '../../../appTypes';
import { PaginatedResponse } from '../../../services/genericService/abstract-crud.service';
import { NavigationService } from '../../../services/navigation-service';

import { FormsModule } from '@angular/forms';
import { TranslatePipe } from '@ngx-translate/core';

@Component({
  selector: 'app-client-moral-list',
  standalone: true,
  imports: [CommonModule, FormsModule, TranslatePipe],
  templateUrl: './client-moral-list.component.html',
  styleUrl: './client-moral-list.component.css'
})
export class ClientMoralListComponent implements OnInit {
  private service = inject(ClientMoralService);
  private navigationService = inject(NavigationService);
  private router = inject(Router);
  
  clients: ClientMoral[] = [];
  filteredClients: ClientMoral[] = [];
  searchTerm: string = '';

  ngOnInit() {
    this.service.findAll(0, 1000, 'createdAt,desc').subscribe((data: PaginatedResponse<ClientMoral>) => {
      this.clients = data.content;
      this.filteredClients = data.content;
    });
  }

  filterClients() {
    const term = this.searchTerm.toLowerCase().trim();
    if (!term) {
      this.filteredClients = this.clients;
      return;
    }
    this.filteredClients = this.clients.filter(c => 
      this.getDisplayName(c).toLowerCase().includes(term) ||
      (c.numeroRegistreCommerce && c.numeroRegistreCommerce.toLowerCase().includes(term)) ||
      String(c.id).includes(term)
    );
  }

  getDisplayName(client: ClientMoral): string {
    return client.nomCommercial || '---';
  }

  getStatusColor(status: string | undefined): string {
    switch (status) {
      case ClientStatus.AML_REQUIRED: return 'bg-yellow-100 text-yellow-800';
      case ClientStatus.VERIFICATION_AML_REQUIRED: return 'bg-amber-100 text-amber-800';
      case ClientStatus.AML_VALIDATED: return 'bg-blue-100 text-blue-800';
      case ClientStatus.INDULGENCE_REQUIRED: return 'bg-orange-100 text-orange-800';
      case ClientStatus.VALIDATED: return 'bg-green-100 text-green-800';
      case ClientStatus.BLOCKED: return 'bg-red-100 text-red-800';
      default: return 'bg-gray-100 text-gray-800';
    }
  }

  navigateToConformity(client: ClientMoral) {
    this.navigationService.navigateToClientConformity(String(client.id));
  }

  navigateToDetails(client: ClientMoral) {
    this.navigationService.navigateToClientDetails(String(client.id));
  }

  navigateToEdit(client: ClientMoral) {
    this.navigationService.navigateToClientEdit(String(client.id));
  }

  navigateToDiligence(client: ClientMoral) {
    this.navigationService.navigateToClientDiligenceResults(String(client.id));
  }
}
